...
plot for [i=0:30] filename(i) using 1:3 ps 0.8  title sprintf("Iteration %d",i)

set output 'GImp_IT.svg'
set title 'G_{Imp,up}(i {/Symbol t}_n)'
filename(n) = sprintf("%d_GImp_IT.out", n)
plot for [i=0:30] filename(i) using 1:2 ps 0.8  title sprintf("Iteration %d",i)
...
plot for [i=0:20] filename(i) using 1:3 ps 0.8 title sprintf("Iteration %d",i)
...
set title 'G_{0,up}(tau)'
filename(n) = sprintf("%d_G0_MF.out", n)
plot for [i=0:20] filename(i) using 1:5 ps 0.8 title sprintf("Iteration %d",i)
