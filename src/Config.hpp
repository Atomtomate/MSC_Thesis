...
namespace DMFT
{
    ...
    static constexpr int _CONFIG_maxMatsFreq = 2048;//4096;//4096;		// frequencies go from maxMatsFreq/2 to maxMatsFreq/2 - 1
    ...
    static constexpr int _CONFIG_maxSBins = 2*8192;//16384;
    ...
    struct Config
    {
        public:
             ...
             */
            Config(const RealT beta, const RealT mu, const RealT U, const RealT D, const int mfCount, const int itCount, const boost::mpi::communicator local, const boost::mpi::communicator world, const bool isGen, const std::string solver, const std::string outDir = ".", std::string additional_info = ""):
                beta(beta), mu(mu), D(D), mfCount(mfCount), itCount(itCount), mfGrid(mfCount), U(U), local(local), world(world), isGenerator(isGen), outDir(outDir), info(additional_info), solver(solver)
        {
